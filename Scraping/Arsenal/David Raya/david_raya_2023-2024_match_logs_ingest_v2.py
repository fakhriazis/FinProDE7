...
# URL untuk David Raya Logs
URL = "https://fbref.com/en/players/98ea5115/matchlogs/2023-2024/David-Raya-Match-Logs"
...
OBJECT_NAME = "david_raya_match_logs_2023_2024_v2.json"  # Mengubah nama file di sini
...
def scrape_match_logs_to_json(url):
    ...
    player_info = {}
    player_name = soup.find("h1").text.strip()  # Mengambil nama pemain dari header
    player_info['player_name'] = player_name

    # Mencari nationality dan position
    info_table = soup.find("table", {"class": "attributes"})
    if info_table:
        rows = info_table.find_all("tr")
        for row in rows:
            header = row.find("th").text.strip()
            if header == "Position":
                player_info['position'] = row.find("td").text.strip()
            elif header == "Nationality":
                player_info['nationality'] = row.find("td").text.strip()
    ...
    for table in tables:
        if "matchlogs" in table.get("id", ""):  
            ...
            # Menyimpan data ke file JSON dengan nama baru
            json_file_path = "david_raya_match_logs_2023_2024_v2.json"  # Mengubah nama file di sini
            output_data = {
                "player_info": player_info,
                "match_logs": match_data
            }
            with open(json_file_path, "w", encoding="utf-8") as json_file:
                json.dump(output_data, json_file, indent=4, ensure_ascii=False)

            print("Data has been saved to 'david_raya_match_logs_2023_2024_v2.json'")
