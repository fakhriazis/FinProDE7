...
OBJECT_NAME = "david_raya_match_logs_2023_2024.json"
...
def scrape_match_logs_to_json(url):
    ...
    for table in tables:
        if "matchlogs" in table.get("id", ""):  # Pastikan tabel memiliki ID mengandung "matchlogs"
            ...
            match_data = []
            for row in rows:
                if row.find("th", {"scope": "row"}):  # Pastikan hanya mengambil baris dengan data
                    row_data = {
            ...
            json_file_path = "david_raya_match_logs_2023_2024.json"
            with open(json_file_path, "w", encoding="utf-8") as json_file:
                json.dump(match_data, json_file, indent=4, ensure_ascii=False)

            print("Data has been saved to 'david_raya_match_logs_2023_2024.json'")
